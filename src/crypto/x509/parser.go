...
func parseName(raw cryptobyte.String) (*pkix.RDNSequence, error) {
	...
	for !raw.Empty() {
		...
		// Keep the raw SET data for validation
		if !raw.ReadASN1Element(&rawSet, cryptobyte_asn1.SET) {
			return nil, errors.New("x509: invalid RDNSequence")
		}

		// Now unwrap the SET for parsing
		if !rawSet.ReadASN1(&set, cryptobyte_asn1.SET) {
			return nil, errors.New("x509: invalid RDNSequence")
		}

		// Store the raw attributes for later verification
		...
		for !set.Empty() {
			...
			// Keep the raw SEQUENCE data for verification
			...
			// Now unwrap the SEQUENCE for parsing
